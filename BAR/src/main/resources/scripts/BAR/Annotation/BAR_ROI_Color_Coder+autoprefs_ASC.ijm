 ...
 *
 * Tiago Ferreira, v.5.4 2017.03.10 + pjl mods 3/13/2017
  ...
  Added option to make a new combined image that combines the labeled image with the legend 10/1/2016
	This version v170313 (updates to AR v.5.4 version)
 */
...
if (roiManager("count")==0 || nResults==0) {
	...
	*/
	...
	if (range.length==1) {
		min= NaN;
		if (useLog) max= log(parseFloat(range[0])) / log(10);
		else max= parseFloat(range[0]);
	} else {
		if (useLog) min= log(parseFloat(range[0])) / log(10);
		else  min= parseFloat(range[0]);
		if (useLog) max= log(parseFloat(range[1])) / log(10);
		else max= parseFloat(range[1]);
	}
...
*/
	...
	if (!useLog) {
		if (0.98*min>arrayMin) minIOR = true;
		else minIOR = false;
		if (max<0.98*arrayMax) maxIOR = true;
		else maxIOR = false;
		if (0.98*min<arrayMin) minOOR = true;
		else minOOR = false;
		if (max>0.98*arrayMax) maxOOR = true;
		else maxOOR = false;
		if (!maxOOR && !minOOR) minmaxLines = false;
	} else {
		if ((min-0.009)>arrayMin) minIOR = true;
		else minIOR = false;
		if (max<(arrayMax-0.009)) maxIOR = true;
		else maxIOR = false;
		if (min<(arrayMin-0.009)) minOOR = true;
		else minOOR = false;
		if ((max-0.009)>arrayMax) maxOOR = true;
		else maxOOR = false;
		if (!maxOOR && !minOOR) minmaxLines = false;
	}
	for (i=0; i<numLabels; i++) {
		...
		if (useLog) rampLabel = pow(10, rampLabel);
		...
		if (minIOR && i==0) {
			...
			if (!useLog) rampExt = removeTrailingZerosAndPeriod(d2s(arrayMin,decPlaces+1)); /* adding 1 to dp ensures that the range is different */
			else rampExt = removeTrailingZerosAndPeriod(d2s(pow(10,arrayMin),decPlaces+1));
			rampLabelString = rampExt + "-" + rampLabelString; 
		}
		if (maxIOR && i==numLabels-1) {
			/*Now add overrun text labels at the top of the ramp if the true data extends above the ramp range */
			if (!useLog) rampExt = removeTrailingZerosAndPeriod(d2s(arrayMax,decPlaces+1));
			else rampExt = removeTrailingZerosAndPeriod(d2s(pow(10,arrayMax),decPlaces+1));
